	...
	**/
	function send($file,$mime=NULL,$kbps=0,$force=TRUE,$name=NULL,$flush=TRUE) {
		...
		if (PHP_SAPI!='cli') {
			...
			header('X-Powered-By: '.Core::instance()->PACKAGE);
		}
		if (!$kbps && $flush) {
			while (ob_get_level())
				ob_end_clean();
		...
		foreach ($_FILES as $name=>$item) {
			...
			foreach ($files as $file) {
				...
				$out[$file['name']]=!$file['error'] &&
					...
					(!$func || $vf->call($func,[$file,$name])!==FALSE) &&
	...
	**/
	function slug($text) {
		return trim(strtolower(preg_replace('/([^\pL\pN])+/u','-',
			trim(strtr($text,Core::instance()->DIACRITICS+$this->diacritics())))),'-');
	}
	...
	**/
	function gzdecode($str) {
		...
		file_put_contents($file=$tmp.'/'.$fw->SEED.'.'.
			$vf->hash(uniqid(NULL,TRUE)).'.gz',$str,LOCK_EX);
		ob_start();
		readgzfile($file);
		$out=ob_get_clean();
		@unlink($file);
