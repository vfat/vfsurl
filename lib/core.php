...
//! Core structure
final class Core extends Prefab	implements ArrayAccess {

	//@{ Detail
	const
		PACKAGE='Grobmart-sys',
		VERSION='0.2-Alpha';
	//@}
	...
	//Parse string dgn key-value pairs buat routing tokens
	function parse($str) {
	...
	//hive variable 'on'=>Return TRUE 
	function checked($key) {
		$ref=&$this->ref($key);
		return $ref=='on';
	}

	//Return TRUE kalo property public
	...
	**/
	...
	function safeString($arg,$tags=NULL) {
		return $this->recursive($arg,
			function($val) use($tags) {
				if ($tags!='*')
					$val=trim(strip_tags($val,
	...
	//regex alphanumeric 
	function alnum(&$var,$tags=NULL){
		return $var=$this->safeString($var,$tags);
	}
	...
	**/
	...
	//Log error; Execute ONERROR handler if defined, else display default error page (klo req sycn HTML, JSON klo AJAX requests)
	...
	function redirect($pattern,$url,$permanent=TRUE) {
		...
		$this->route($pattern,function($vf) use($url,$permanent) {
			$vf->reroute($url,$permanent);
		});
	}
	...
	//config di .ini file settings;kalo ada optional 2nd arg, pake template string
	...
	//Read file (dengan opsi apply Unix LF buat ending standard)
	...
	//Dump expression dgn syntax highlighting
	function dump($expr) {
		echo $this->highlight($this->stringify($expr));
	}

	//Return path (dan query parameter) tergantung base directory
	function rel($url) {
	...
	//Convenience method buat checking hive key
	function offsetexists($key) {
		return $this->exists($key);
	}

	//Convenience method buat assigning hive value
	function offsetset($key,$val) {
		return $this->set($key,$val);
	}

	//Convenience method buat retrieving hive value
	function &offsetget($key) {
		$val=&$this->ref($key);
		return $val;
	}

	//Convenience method buat removing hive key
	function offsetunset($key) {
		$this->clear($key);
	}

	//Alias buat offsetexists()
	function __isset($key) {
		return $this->offsetexists($key);
	}

	//Alias buat offsetset()
	function __set($key,$val) {
		return $this->offsetset($key,$val);
	}

	//Alias buat offsetget()
	function &__get($key) {
		$val=&$this->offsetget($key);
		return $val;
	}

	//Alias buat offsetunset()
...
class Cache extends Prefab {
	...
	//Return timestamp n TTL cache entry or FALSE klo ga ada
	function exists($key,&$val=NULL) {
		$vf=Core::instance();
		if (!$this->dsn)
			return FALSE;
		...
		switch ($parts[0]) {
			...
			case 'xcache':
				...
				break;
			case 'folder':
				$raw=$vf->read($parts[1].$ndx);
				break;
		}
		if (!empty($raw)) {
			list($val,$time,$ttl)=(array)$vf->unserialize($raw);
	...
	function set($key,$val,$ttl=0) {
		...
		$data=$vf->serialize([$val,$time,$ttl]);
		$parts=explode('=',$this->dsn,2);
		switch ($parts[0]) {
			case 'apc':
			case 'apcu':
			...
			case 'folder':
				return $vf->write($parts[1].$ndx,$data);
		}
		return FALSE;
	}
	...
	function load($dsn,$seed=NULL) {
		...
		if ($dsn=trim($dsn)) {
			...
			elseif (preg_match('/^memcache=(.+)/',$dsn,$parts) &&
				...
				foreach ($vf->split($parts[1]) as $server) {
			...
			if (empty($this->ref) && !preg_match('/^folder\h*=/',$dsn))
				$dsn=($grep=preg_grep('/^(apc|wincache|xcache)/',
					array_map('strtolower',get_loaded_extensions())))?
						...
						('folder='.$vf->TEMP.'cache/');
			if (preg_match('/^folder\h*=\h*(.+)/',$dsn,$parts) &&
				!is_dir($parts[1]))
				mkdir($parts[1],Core::MODE,TRUE);
		}
		$this->prefix=$seed?:$vf->SEED;
		return $this->dsn=$dsn;
	}
...
class View extends Prefab {
	...
	function esc($arg) {
		$vf=Core::instance();
		return $vf->recursive($arg,
			function($val) use($vf) {
				return is_string($val)?$vf->encode($val):$val;
			}
		);
	}

	//Decode HTML entities ke raw
	function raw($arg) {
		$vf=Core::instance();
		return $vf->recursive($arg,
			function($val) use($vf) {
				return is_string($val)?$vf->decode($val):$val;
			}
		);
	}
	...
	protected function sandbox(array $hive=NULL,$mime=NULL) {
		...
		if ($this->level<1 || $implicit) {
			...
			if (isset($hive['ALIASES']))
				$hive['ALIASES']=$vf->build($hive['ALIASES']);
		}
		$this->temp=$hive;
		unset($vf,$hive,$implicit,$mime);
		extract($this->temp);
		$this->temp=NULL;
		$this->level++;
		ob_start();
	...
	function render($file,$mime='text/html',array $hive=NULL,$ttl=0) {
		$vf=Core::instance();
		$cache=Cache::instance();
		if ($cache->exists($hash=$vf->hash($file),$data))
			return $data;
		foreach ($vf->split($vf->UI) as $dir)
			if (is_file($this->file=$vf->fixslashes($dir.$file))) {
				...
				if(isset($this->trigger['afterrender']))
					foreach($this->trigger['afterrender'] as $func)
						$data=$vf->call($func,$data);
				if ($ttl)
					$cache->set($hash,$data,$ttl);
				return $data;
			}
		user_error(sprintf(Core::E_Open,$file),E_USER_ERROR);
	}
...
class Preview extends View {
	...
	protected
		...
		$filter=[
			...
			'esc'=>'$this->esc',
			'raw'=>'$this->raw',
			'alias'=>'Core::instance()->alias',
			'format'=>'Core::instance()->format'
		];
	...
	**/
	function token($str) {
		$str=trim(preg_replace('/\{\{(.+?)\}\}/s',trim('\1'),
			Core::instance()->compile($str)));
		if (preg_match('/^(.+)(?<!\|)\|((?:\h*\w+(?:\h*[,;]?))+)$/s',
			$str,$parts)) {
			$str=trim($parts[1]);
			foreach (Core::instance()->split($parts[2]) as $func)
	...
	**/
	function resolve($node,array $hive=NULL,$ttl=0,$persist=FALSE) {
		...
		$cache=Cache::instance();
		if ($ttl || $persist)
			$hash=$vf->hash($vf->serialize($node));
		if ($ttl && $cache->exists($hash,$data))
			return $data;
		if ($persist) {
			if (!is_dir($tmp=$vf->TEMP))
				mkdir($tmp,Core::MODE,TRUE);
			if (!is_file($this->file=($tmp.
				$vf->SEED.'.'.$hash.'.php')))
				$vf->write($this->file,$this->build($node));
			if (isset($_COOKIE[session_name()]) &&
				!headers_sent() && session_status()!=PHP_SESSION_ACTIVE)
				session_start();
			$vf->sync('SESSION');
	...
	**/
	function render($file,$mime='text/html',array $hive=NULL,$ttl=0) {
		...
		if (!is_dir($tmp=$vf->TEMP))
			mkdir($tmp,Core::MODE,TRUE);
		foreach ($vf->split($vf->UI) as $dir) {
			if ($cache->exists($hash=$vf->hash($dir.$file),$data))
				return $data;
			if (is_file($view=$vf->fixslashes($dir.$file))) {
				if (!is_file($this->file=($tmp.
					$vf->SEED.'.'.$vf->hash($view).'.php')) ||
					filemtime($this->file)<filemtime($view)) {
					$text=$this->parse($vf->read($view));
					$vf->write($this->file,$this->build($text));
				}
				if (isset($_COOKIE[session_name()]) &&
					!headers_sent() && session_status()!=PHP_SESSION_ACTIVE)
					session_start();
...
class ISO extends Prefab {
	...
	**/
	function languages() {
		return \Core::instance()->constants($this,'LC_');
	}
	...
	**/
	function countries() {
		return \Core::instance()->constants($this,'CC_');
	}
